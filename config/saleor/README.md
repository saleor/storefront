...
2. Export Configurator seed + Paper snapshot: `pnpm content:export-seed` (runs verify automatically)
3. Verify (CI / `pnpm test:run`): `pnpm content:verify-seed`
4. Deploy when bootstrapping Saleor: Paper app init (merchants) or `pnpm configurator:storefront-content:deploy` (dev/advanced)
5. When the snapshot version changes, bump the vendored copy in [`saleor-paper-app`](https://github.com/saleor/saleor-paper-app)
...
**Paper app snapshot:** `config/saleor/storefront-content.snapshot.json` is the machine-readable SoT the Paper Dashboard app applies on init. Regenerated by `content:export-seed`; do not maintain a fork of the schema in the app.

### Required vs optional Models (blank merchant)

| Tier                              | Models                     | Notes                                                       |
| --------------------------------- | -------------------------- | ----------------------------------------------------------- |
| Recommended for Dashboard editing | policies, chrome, homepage | Site-wide copy + free-shipping facts                        |
| Recommended                       | cart, checkout             | Brand voice on bag/checkout                                 |
| Optional                          | products                   | PLP title/description only                                  |
| Skip                              | —                          | Forks / code-only: uncheck init, or `CONTENT_PROVIDER=code` |

Paper app init seeds **all six** when “Add storefront Models” is checked.
...
Configurator / bootstrap scripts seed **English** attribute values via the exported `models:` block (from `defaults.ts`). Non-default locales live in separate YAML files used **once** to populate a fresh Saleor instance:
...
These fixtures are **demo/bootstrap samples** — they may lag neutralized English defaults in `defaults.ts`. Prefer editing locale copy in **Dashboard → Models** after bootstrap. When re-running translation deploy on a shop that already has Dashboard edits, expect overwrites.
...
Fastest one-off: create the six **Storefront — \*** types/models by hand (Policies, Chrome, Products, Homepage, Cart, Checkout). `CONTENT_PROVIDER` already defaults to `saleor`.

## After deploy

1. Dashboard → **Models** → tabs **Storefront — Policies / Chrome / Products / Homepage / Cart / Checkout**.
2. Prefer bootstrapping via the **Paper app** init screen (“Add storefront Models”). Configurator (`pnpm configurator:storefront-content:deploy`) remains the advanced/dev path.
3. Optional: set `CONTENT_PROVIDER=code` in `.env.local` only if you want code defaults without Models.
4. Restart the dev server after changing `CONTENT_PROVIDER`.
