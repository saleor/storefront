	...
	return (
		<div
			className={clsx(
				...
				{
					"border-red-700 bg-red-950": errors.length > 0,
					"border-amber-700 bg-amber-950": errors.length === 0 && warnings.length > 0,
					"border-blue-700 bg-blue-950": errors.length === 0 && warnings.length === 0 && infos.length > 0,
				},
				className,
			)}
			role="alert"
				...
				<div className="flex-shrink-0 pt-0.5">
					<AlertTriangleIcon
						className={clsx("h-5 w-5", {
							...
							"text-amber-400": errors.length === 0 && warnings.length > 0,
							"text-blue-400": errors.length === 0 && warnings.length === 0,
						})}
					/>
				</div>
				<div className="flex-1">
					<h3
						className={clsx("text-sm font-semibold", {
							"text-red-200": errors.length > 0,
							"text-amber-200": errors.length === 0 && warnings.length > 0,
							"text-blue-200": errors.length === 0 && warnings.length === 0,
						})}
					>
						{title}
					</h3>
