...
const paymentIcons: Record<string, React.ReactNode> = {
	visa: (
		<svg className="h-5 w-8 text-secondary-400" viewBox="0 0 38 24" fill="currentColor">
			<path
	...
	mpesa: <span className="text-xs text-secondary-500">M-Pesa</span>,
};
...
export async function Footer({ channel }: { channel: string }) {
	const [footerLinks, channels, shopInfo] = await Promise.all([
			...
			<div className="mx-auto max-w-7xl px-4 lg:px-8">
				...
				<div className="grid grid-cols-1 gap-8 py-12 md:grid-cols-2 lg:grid-cols-4">
					...
					<div className="lg:col-span-1">
						<h3 className="mb-4 text-lg font-bold text-white">{shopInfo.name}</h3>
						<p className="mb-4 text-sm leading-relaxed text-secondary-300">{shopInfo.description}</p>
					...
					{footerLinks.menu?.items?.map((item) => (
						<div key={item.id}>
							<h3 className="mb-4 text-sm font-semibold uppercase tracking-wider text-white">{item.name}</h3>
										...
										return (
											<li key={child.id}>
												<LinkWithChannel
													...
													className="text-sm text-secondary-300 transition-colors hover:text-white"
												>
													{child.category.name}
												</LinkWithChannel>
											</li>
						...
						<div className="flex items-center gap-6">
							<div className="flex items-center gap-2">
								{shopInfo.paymentMethods.map((method) => (
									<span key={method}>
										{paymentIcons[method] || <span className="text-xs text-secondary-500">{method}</span>}
									</span>
								))}
							</div>
						</div>
